...
#
# `function claude {{` and not `claude() {{`: if an alias named `claude` already
# exists (e.g. `alias claude='claude --effort ultracode'`), zsh expands it while
# PARSING the `claude()` line — "defining function based on alias" — and aborts
# with a parse error. The function is then never defined, the alias survives, and
# `claude` runs the real binary with no proxy. The `function` keyword suppresses
# that expansion; it is accepted by both zsh and bash.
function claude {{
    # Check if proxy is already listening on 9999 (nc -z works in bash and zsh)
