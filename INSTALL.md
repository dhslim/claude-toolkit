...
Read ~/.claude/CLAUDE.md (create it if it doesn't exist). Append the two sections below ("MongoDB Atlas" and "Response timestamp"). For each section, if it already exists, update it to match; otherwise append it. Preserve all other existing content.
...
## Response timestamp
- Each turn, a `UserPromptSubmit` hook (`hook_inject_time.py`, wired by the settings template in step 5) injects a `<current-time>YYYY-MM-DD HH:MM:SS KST</current-time>` line into the context.
- End every response with that exact timestamp on its own line as plain text, e.g. `2026-06-07 14:32:01 KST`.
